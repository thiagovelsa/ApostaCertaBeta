...
```
API/
├── AGENTS.md                           # Diretrizes do repositório (workflow/padrões)
...
**Nota (por time):**
- Stats por time via `liveData.lineUp[].stat[]` só aparecem quando a partida já foi disputada.
- Para cada `stat`, a VStats costuma fornecer:
  - `value`: total do jogo
  - `fh`: 1º tempo (first half)
  - `sh`: 2º tempo (second half)

Isso permite recortes `integral`/`1T`/`2T` sem depender de arrays de 6 posições.
